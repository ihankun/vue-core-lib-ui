          ...
          <a-dropdown :trigger="['click']" overlayClassName="header-notifications-dropdown" :getPopupContainer="() => wrapper">
            <!-- <a-badge count="4"> -->
              <a class="ant-dropdown-link" @click="e => e.preventDefault()">
                <svg-icon class="header-icon" :icon-name= "`notification`"></svg-icon>
              </a>
            <!-- </a-badge> -->
              ...
              <a-list-item slot="renderItem" slot-scope="item">
                <a-list-item-meta>
                  <template #description>
                    <svg-icon class="header-icon" :icon-name= "`time`"></svg-icon>
                    <span>{{ item.time }}</span>
                  </template>
                  <a slot="title" href="#">{{ item.title }}</a>
                  <a-avatar v-if="item.img" slot="avatar" shape="square" :src="item.img"></a-avatar>
                  <a-avatar v-else slot="avatar" shape="square" v-html="item.svg"></a-avatar>
                </a-list-item-meta>
              </a-list-item>
            </a-list>
          </a-dropdown>
          <a type="link" ref="secondarySidebarTriggerBtn" @click="$emit('toggleSettingsDrawer', true)">
            <svg-icon class="header-icon" :icon-name= "`setting`"></svg-icon>
          </a>
          <a type="link" class="sidebar-toggler" @click="$emit('toggleSidebar', ! sidebarCollapsed) , resizeEventHandler()">
            <svg-icon class="header-icon" :icon-name= "`list`"></svg-icon>
          </a>
          <router-link to="/sign-in" class="btn-sign-in" @click="e => e.preventDefault()">
            <svg-icon class="header-icon" :icon-name= "`sign-in`"></svg-icon>
            <span>登录</span>
          </router-link>
          <a-input-search class="header-search" :class="searchLoading ? 'loading' : ''"
            placeholder="搜索…" @search="onSearch" :loading='searchLoading'
          >
            <svg-icon class="header-icon" :icon-name= "`search`"></svg-icon>
          </a-input-search>
        </a-col>
      </a-row>
    </a-layout-header>
...
export default class DashboardHeader extends Vue {
  ...
  @Prop({ type: Array, default: () => notificationsData }) notificationsData !: boolean
  private top: number = top
  // private notificationsData: any[] = notificationsData
  private wrapper: any = wrapper
  private searchLoading: boolean = searchLoading
  mounted() {
